...
Version des beschriebenen Stands: 2.20.2
...
Die gesamte Wall (Fly und Grid: Medien, Hintergrund, Banner, QR, Status) lässt sich drehen: unter `/admin` Reiter Wand, Abschnitt Spezial, Auswahl Horizontal 0° (Standard), Porträt 90°, Horizontal gedreht 180°, Porträt gedreht 270°. Wert `wall_display_rotation` in der Projekt-Config. Für Geräte ohne echten Hochkant-Modus. Die Wall-HTML kommt ohne Browser-Cache; `wall-rotate.js`/`wall-rotate.css` mit Versionsparameter, damit die alte Auswahl „Anzeige“ auf `/wall` nicht aus dem Cache bleibt.
